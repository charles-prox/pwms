import React from "react";

interface IconProps extends React.SVGProps<SVGSVGElement> {}

export const DashboardIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            fill="currentColor"
            viewBox="0 0 48 48"
            xmlns="http://www.w3.org/2000/svg"
        >
            <title>dashboard-tile-solid</title>
            <g id="Layer_2">
                <g id="invisible_box">
                    <rect width="48" height="48" fill="none" />
                </g>
                <g id="icons_Q2">
                    <g>
                        <path d="M20,30H8a2,2,0,0,0-2,2V42a2,2,0,0,0,2,2H20a2,2,0,0,0,2-2V32a2,2,0,0,0-2-2Z" />
                        <path d="M20,4H8A2,2,0,0,0,6,6V24a2,2,0,0,0,2,2H20a2,2,0,0,0,2-2V6a2,2,0,0,0-2-2Z" />
                        <path d="M40,4H28a2,2,0,0,0-2,2V16a2,2,0,0,0,2,2H40a2,2,0,0,0,2-2V6a2,2,0,0,0-2-2Z" />
                        <path d="M40,22H28a2,2,0,0,0-2,2V42a2,2,0,0,0,2,2H40a2,2,0,0,0,2-2V24a2,2,0,0,0-2-2Z" />
                    </g>
                </g>
            </g>
        </svg>
    );
};

export const UsersIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <rect width="24" height="24" fill="none" />
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M5 9.5C5 7.01472 7.01472 5 9.5 5C11.9853 5 14 7.01472 14 9.5C14 11.9853 11.9853 14 9.5 14C7.01472 14 5 11.9853 5 9.5Z"
                fill="currentColor"
            />
            <path
                d="M14.3675 12.0632C14.322 12.1494 14.3413 12.2569 14.4196 12.3149C15.0012 12.7454 15.7209 13 16.5 13C18.433 13 20 11.433 20 9.5C20 7.567 18.433 6 16.5 6C15.7209 6 15.0012 6.2546 14.4196 6.68513C14.3413 6.74313 14.322 6.85058 14.3675 6.93679C14.7714 7.70219 15 8.5744 15 9.5C15 10.4256 14.7714 11.2978 14.3675 12.0632Z"
                fill="currentColor"
            />
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M4.64115 15.6993C5.87351 15.1644 7.49045 15 9.49995 15C11.5112 15 13.1293 15.1647 14.3621 15.7008C15.705 16.2847 16.5212 17.2793 16.949 18.6836C17.1495 19.3418 16.6551 20 15.9738 20H3.02801C2.34589 20 1.85045 19.3408 2.05157 18.6814C2.47994 17.2769 3.29738 16.2826 4.64115 15.6993Z"
                fill="currentColor"
            />
            <path
                d="M14.8185 14.0364C14.4045 14.0621 14.3802 14.6183 14.7606 14.7837V14.7837C15.803 15.237 16.5879 15.9043 17.1508 16.756C17.6127 17.4549 18.33 18 19.1677 18H20.9483C21.6555 18 22.1715 17.2973 21.9227 16.6108C21.9084 16.5713 21.8935 16.5321 21.8781 16.4932C21.5357 15.6286 20.9488 14.9921 20.0798 14.5864C19.2639 14.2055 18.2425 14.0483 17.0392 14.0008L17.0194 14H16.9997C16.2909 14 15.5506 13.9909 14.8185 14.0364Z"
                fill="currentColor"
            />
        </svg>
    );
};

export const BoxStore: React.FC<IconProps> = (props) => {
    return (
        <svg fill="currentcolor" {...props} viewBox="0 0 612 612">
            <g>
                <path
                    d="M598.551,135.131l-70.8-88.827c-6.688-8.391-16.835-13.279-27.564-13.279H160.682c-10.73,0-20.876,4.888-27.563,13.278
		l-70.802,88.828c-7.397,9.28-13.449,26.583-13.449,38.449v64.615c15.441-12.739,35.22-20.402,56.755-20.402h31.626
		c49.25,0,89.318,40.068,89.318,89.318v52.221c24.363,6.549,42.736,30.018,42.736,55.248v109.621h298.635
		c24.334,0,44.061-19.729,44.061-44.061L612,173.58C612,161.713,605.948,144.411,598.551,135.131z M85.454,134.379l23.447-29.417
		h159.975l15.786-25.956H129.589L146.9,57.29c3.363-4.22,8.386-6.639,13.782-6.639h339.504c5.396,0,10.419,2.419,13.782,6.639
		l17.31,21.718H373.444l8.812,25.956h169.711l23.446,29.417H85.454z M217.931,384.443c-5.918-1.041-11.859-1.885-17.801-2.732v-74.6
		c0-34.676-28.208-62.884-62.884-62.884h-31.622c-34.677,0-62.884,28.208-62.884,62.884v74.6
		c-5.942,0.848-11.884,1.691-17.803,2.734C11.116,387.021,0.002,400.861,0,414.576c0,41.947,0,83.893,0,125.838
		c0.001,13.719,11.117,27.562,24.936,30.133c63.839,11.236,129.156,11.236,192.995,0c13.821-2.576,24.932-16.416,24.936-30.133
		c0-41.945,0-83.891,0-125.838C242.866,400.861,231.748,387.016,217.931,384.443z M84.662,307.111
		c0-11.561,9.4-20.961,20.962-20.961h31.623c11.561,0,20.961,9.4,20.961,20.961v70.254c-24.493-1.621-49.053-1.623-73.545-0.002
		L84.662,307.111L84.662,307.111z M137.592,514.361h-32.317l8.69-35.195c-7.343-2.963-12.536-10.129-12.536-18.531
		c0-11.049,8.957-20.006,20.004-20.006c11.049,0,20.005,8.957,20.005,20.006c0,8.402-5.194,15.568-12.537,18.531L137.592,514.361z"
                />
            </g>
        </svg>
    );
};

export const BoxSearch: React.FC<IconProps> = (props) => {
    return (
        <svg
            fill="currentColor"
            version="1.1"
            {...props}
            viewBox="0 0 611.998 611.998"
        >
            <g>
                <path
                    d="M598.007,109.07l-73.663-92.419c-6.958-8.729-17.515-13.815-28.679-13.815H169.943c-11.164,0-21.72,5.085-28.678,13.815
		L67.601,109.07c-7.696,9.655-13.992,27.656-13.992,40.002v152.604c28.321-47.217,79.863-78.988,138.814-78.988
		c89.294,0,161.934,72.641,161.934,161.926c0,52.824-25.536,99.674-64.787,129.25h276.585c25.318,0,45.843-20.523,45.843-45.842
		V149.072C611.999,136.726,605.703,118.725,598.007,109.07z M91.675,108.287l24.395-30.605h120.598l16.425-27.005H137.594
		l18.011-22.596c3.499-4.391,8.726-6.908,14.339-6.908h325.722c5.614,0,10.841,2.518,14.34,6.908l18.011,22.596H409.643
		l9.169,27.005h130.729l24.394,30.605H91.675z M192.429,250.195c-74.24,0-134.423,60.184-134.423,134.424
		c0,31.756,11.076,60.889,29.489,83.893L8.52,554.762c-12.106,13.227-11.204,33.768,2.023,45.881
		c6.236,5.701,14.086,8.52,21.922,8.52c8.788,0,17.554-3.551,23.959-10.541l82.888-90.523c16.298,7.021,34.244,10.945,53.117,10.945
		c74.24,0,134.423-60.184,134.423-134.424C326.853,310.381,266.668,250.195,192.429,250.195z M192.429,473.137
		c-48.888,0-88.519-39.631-88.519-88.518c0-48.889,39.631-88.52,88.519-88.52s88.519,39.631,88.519,88.52
		C280.948,433.506,241.316,473.137,192.429,473.137z"
                />
            </g>
        </svg>
    );
};

export const BoxReturn: React.FC<IconProps> = (props) => {
    return (
        <svg fill="currentcolor" {...props} viewBox="0 0 612 612">
            <g>
                <path
                    d="M540.632,286.969l-63.029-131.455c-5.957-7.447-14.973-11.838-24.537-11.838h-20.227l-18.344,15.679h38.57
		c4.859,0,9.328,2.196,12.309,5.958l54.639,121.027H91.985l54.64-121.027c2.979-3.763,7.447-5.958,12.308-5.958h38.648
		l-18.344-15.679h-20.304c-9.564,0-18.58,4.391-24.538,11.838L71.368,286.969c-6.507,8.231-11.916,23.674-11.916,34.179v251.654
		c0,21.637,17.56,39.197,39.197,39.197h414.701c21.637,0,39.197-17.56,39.197-39.197V321.148
		C552.548,310.644,547.138,295.201,540.632,286.969z M427.274,431.773l24.381-42.176c2.43-4.154,8.465-4.154,10.896,0l24.381,42.176
		c2.43,4.233-0.629,9.486-5.488,9.486h-10.191v78.628c0,3.528-2.822,6.271-6.271,6.271h-15.758c-3.449,0-6.271-2.743-6.271-6.271
		v-78.629h-10.191C427.901,441.259,424.845,436.007,427.274,431.773z M332.966,431.773l24.381-42.176
		c2.432-4.154,8.467-4.154,10.896,0l24.381,42.176c2.43,4.233-0.627,9.486-5.486,9.486h-10.191v78.628
		c0,3.528-2.822,6.271-6.271,6.271h-15.758c-3.449,0-6.271-2.743-6.271-6.271v-78.629h-10.191
		C333.595,441.259,330.536,436.007,332.966,431.773z M498.847,561.75c0,1.725-1.41,3.136-3.135,3.136H322.228
		c-1.727,0-3.137-1.41-3.137-3.136v-9.485c0-1.725,1.41-3.136,3.137-3.136h173.484c1.725,0,3.135,1.41,3.135,3.136V561.75z
		 M371.054,83.117h75.191c7.285,0,10.631,9.07,5.09,13.801L311.077,216.685c-2.932,2.504-7.25,2.504-10.181,0L160.66,96.918
		c-5.539-4.731-2.194-13.801,5.091-13.801h75.188c4.33,0,7.839-3.51,7.839-7.84V7.84c0-4.33,3.51-7.84,7.84-7.84h98.759
		c4.33,0,7.84,3.51,7.84,7.84v67.438C363.216,79.607,366.726,83.117,371.054,83.117z"
                />
            </g>
        </svg>
    );
};

export const BoxDispose: React.FC<IconProps> = (props) => {
    return (
        <svg fill="currentcolor" {...props} viewBox="0 0 612 612">
            <g>
                <path
                    d="M181.298,386.264c-29.77,0-53.915,24.145-53.915,53.827c0,29.769,24.145,53.914,53.915,53.914
		c29.683,0,53.827-24.146,53.827-53.914C235.125,410.409,210.981,386.264,181.298,386.264z M181.298,467.091
		c-14.885,0-27-12.115-27-27c0-14.798,12.115-26.914,27-26.914c14.797,0,26.913,12.115,26.913,26.914
		C208.211,454.976,196.096,467.091,181.298,467.091z M134.888,386.287H14.228c-4.779,0-8.653,3.874-8.653,8.653v26.256
		c0,4.779,3.874,8.653,8.653,8.653h96.714C113.468,412.524,122.212,397.219,134.888,386.287z M462.981,386.264
		c-29.684,0-53.827,24.145-53.827,53.827c0,29.769,24.144,53.914,53.827,53.914c29.769,0,53.826-24.146,53.826-53.914
		C516.808,410.409,492.75,386.264,462.981,386.264z M462.981,467.091c-14.885,0-26.914-12.115-26.914-27
		c0-14.798,12.029-26.914,26.914-26.914c14.884,0,26.913,12.115,26.913,26.914C489.895,454.976,477.865,467.091,462.981,467.091z
		 M603.347,386.264h-21.029V268.658c0-13.759-4.154-27.259-11.856-38.683l-52.701-78.057
		c-12.809-19.039-34.356-30.462-57.375-30.462h-81.692c-9.52,0-17.308,7.702-17.308,17.308v247.5H227.683
		c12.635,10.99,21.375,26.222,23.885,43.615h141.144c4.933-34.355,34.529-60.923,70.27-60.923c35.739,0,65.336,26.567,70.355,60.923
		h70.01c4.847,0,8.653-3.895,8.653-8.654v-26.307C612,390.158,608.192,386.264,603.347,386.264z M517.153,245.552H409.327
		c-4.76,0-8.654-3.808-8.654-8.654v-59.797c0-4.76,3.896-8.654,8.654-8.654h65.683c2.855,0,5.539,1.384,7.097,3.634l42.145,59.884
		C528.317,237.677,524.163,245.552,517.153,245.552z M320.192,368.956H17.307C7.749,368.956,0,361.208,0,351.649v-77.885
		c0-9.559,7.749-17.308,17.307-17.308h302.885c9.559,0,17.308,7.749,17.308,17.308v77.885
		C337.5,361.208,329.751,368.956,320.192,368.956z M138.462,230.495H17.307C7.749,230.495,0,222.745,0,213.187v-77.884
		c0-9.559,7.749-17.308,17.307-17.308h121.154c9.559,0,17.308,7.749,17.308,17.308v77.884
		C155.77,222.745,148.021,230.495,138.462,230.495z M320.192,230.495H199.039c-9.559,0-17.308-7.749-17.308-17.307v-77.885
		c0-9.559,7.749-17.307,17.308-17.307h121.154c9.559,0,17.308,7.749,17.308,17.307v77.885
		C337.5,222.745,329.751,230.495,320.192,230.495z"
                />
            </g>
        </svg>
    );
};

export const ArrowRightIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M8.51192 4.43057C8.82641 4.161 9.29989 4.19743 9.56946 4.51192L15.5695 11.5119C15.8102 11.7928 15.8102 12.2072 15.5695 12.4881L9.56946 19.4881C9.29989 19.8026 8.82641 19.839 8.51192 19.5695C8.19743 19.2999 8.161 18.8264 8.43057 18.5119L14.0122 12L8.43057 5.48811C8.161 5.17361 8.19743 4.70014 8.51192 4.43057Z"
                fill="currentColor"
            />
        </svg>
    );
};

export const ReportsIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            fill="currentColor"
            {...props}
            version="1.1"
            id="Layer_1"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
        >
            <g>
                <g>
                    <path
                        d="M490.75,469.3h-448v-448c0-11.7-9.6-21.3-21.4-21.3S0.05,9.6,0.05,21.3v469.3c0,11.8,9.5,21.4,21.3,21.4h469.3
			c11.8,0,21.3-9.6,21.3-21.3C511.95,479,502.55,469.3,490.75,469.3z"
                    />
                </g>
            </g>
            <g>
                <g>
                    <path
                        d="M448.05,0h-85.3c-11.8,0-21.3,9.5-21.3,21.3V128h-42.7v-21.3c0-11.8-9.5-21.3-21.3-21.3h-85.4c-11.8,0-21.3,9.5-21.3,21.3
			v106.7h-64c-11.8,0-21.3,9.5-21.3,21.3v170.7c-0.1,11.7,9.5,21.3,21.3,21.3h85.3h85.3h85.3h85.4c11.8,0,21.3-9.6,21.3-21.3v-384
			C469.35,9.5,459.85,0,448.05,0z M170.75,384h-42.7V256h42.7V384z M341.45,384h-42.7V170.7h42.7V384z"
                    />
                </g>
            </g>
        </svg>
    );
};

export const OfficesIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            fill="currentColor"
            {...props}
            version="1.1"
            id="Layer_1"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512.018 512.018"
        >
            <g>
                <g>
                    <path
                        d="M504.713,85.906l-256-85.333c-3.264-1.131-6.827-0.555-9.621,1.451c-2.795,2.005-4.416,5.227-4.416,8.661v120.533
			L7.027,213.991c-4.224,1.536-7.019,5.525-7.019,10.027v277.333c0,5.888,4.779,10.667,10.667,10.667h490.667
			c5.888,0,10.667-4.779,10.667-10.667V96.018C512.009,91.431,509.065,87.356,504.713,85.906z M106.676,458.684
			c0,5.888-4.779,10.667-10.667,10.667H53.342c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667
			h42.667c5.888,0,10.667,4.779,10.667,10.667V458.684z M106.676,330.684c0,5.888-4.779,10.667-10.667,10.667H53.342
			c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667
			V330.684z M213.342,458.684c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333
			c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667V458.684z M213.342,330.684
			c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667
			h42.667c5.888,0,10.667,4.779,10.667,10.667V330.684z M362.676,458.684c0,5.888-4.779,10.667-10.667,10.667h-42.667
			c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667
			V458.684z M362.676,330.684c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333
			c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667V330.684z M362.676,202.684
			c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667
			h42.667c5.888,0,10.667,4.779,10.667,10.667V202.684z M448.009,458.684c0,5.888-4.779,10.667-10.667,10.667h-42.667
			c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667
			V458.684z M448.009,330.684c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333
			c0-5.888,4.779-10.667,10.667-10.667h42.667c5.888,0,10.667,4.779,10.667,10.667V330.684z M448.009,202.684
			c0,5.888-4.779,10.667-10.667,10.667h-42.667c-5.888,0-10.667-4.779-10.667-10.667v-85.333c0-5.888,4.779-10.667,10.667-10.667
			h42.667c5.888,0,10.667,4.779,10.667,10.667V202.684z"
                    />
                </g>
            </g>
        </svg>
    );
};

export const RdsIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <g id="style=fill">
                <g id="document">
                    <path
                        id="Subtract"
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M8 1.25C4.82436 1.25 2.25 3.82436 2.25 7V17C2.25 20.1756 4.82436 22.75 8 22.75H16C19.1756 22.75 21.75 20.1756 21.75 17V7C21.75 3.82436 19.1756 1.25 16 1.25H8ZM8 7.44995C7.58579 7.44995 7.25 7.78574 7.25 8.19995C7.25 8.61416 7.58579 8.94995 8 8.94995H16C16.4142 8.94995 16.75 8.61416 16.75 8.19995C16.75 7.78574 16.4142 7.44995 16 7.44995H8ZM7.25 12.2C7.25 11.7857 7.58579 11.45 8 11.45H16C16.4142 11.45 16.75 11.7857 16.75 12.2C16.75 12.6142 16.4142 12.95 16 12.95H8C7.58579 12.95 7.25 12.6142 7.25 12.2ZM9 15.45C8.58579 15.45 8.25 15.7857 8.25 16.2C8.25 16.6142 8.58579 16.95 9 16.95H15C15.4142 16.95 15.75 16.6142 15.75 16.2C15.75 15.7857 15.4142 15.45 15 15.45H9Z"
                        fill="currentColor"
                    />
                </g>
            </g>
        </svg>
    );
};

export const LocationIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            viewBox="-3 0 32 32"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
        >
            <title>location 2</title>
            <desc>Created with Sketch Beta.</desc>
            <defs></defs>
            <g
                id="Page-1"
                stroke="none"
                strokeWidth="1"
                fill="none"
                fillRule="evenodd"
            >
                <g
                    id="Icon-Set-Filled"
                    transform="translate(-157.000000, -413.000000)"
                    fill="currentColor"
                >
                    <path
                        d="M182.717,425.224 L179.737,419.282 C179.524,419.069 179.241,418.982 178.962,419 L159,419 C157.896,419 157,419.896 157,421 L157,431 C157,432.104 157.896,433 159,433 L179,432.976 C179,432.976 179.534,432.89 179.737,432.688 L182.717,426.745 C182.942,426.325 182.972,426.188 183.002,425.984 C183.016,425.711 182.927,425.604 182.717,425.224 L182.717,425.224 Z M163,441 C163,443.209 164.791,445 167,445 L169,445 C171.209,445 173,443.209 173,441 L173,435 L163,435 L163,441 L163,441 Z M169,413 L167,413 C164.791,413 163,414.791 163,417 L173,417 C173,414.791 171.209,413 169,413 L169,413 Z"
                        id="location-2"
                    ></path>
                </g>
            </g>
        </svg>
    );
};

export const StorageEntryIcon: React.FC<IconProps> = (props) => {
    return (
        <svg
            {...props}
            viewBox="0 0 16 16"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
        >
            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
            <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
                {" "}
                <path fill="currentColor" d="M4 9h4v2h-4v-2z"></path>{" "}
                <path
                    fill="currentColor"
                    d="M16 2h-1v-2h-10v2h-2v1.25l-0.6 0.75h-1.4v1.75l-1 1.25v9h12l4-5v-9zM2 5h8v2h-8v-2zM11 15h-10v-7h10v7zM12 7h-1v-3h-7v-1h8v4zM14 4.5l-1 1.25v-3.75h-7v-1h8v3.5z"
                ></path>{" "}
            </g>
        </svg>
    );
};
